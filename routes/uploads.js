...
// @access  Admin/SuperAdmin
router.patch('/:id/status', [auth, isAdminOrSuperAdmin], async (req, res, next) => {
  try {
...
router.get('/history', auth, async (req, res, next) => {
  try {
    const { uploads, totalPages, currentPage } = await uploadService.getUploadHistory(
      req.user.id,
      req.query
    );
    res.json({
...
router.delete('/last', auth, async (req, res, next) => {
  try {
    ...
    res.json({ msg: 'Last upload discarded successfully.', upload: discardedUpload });
  } catch (err) {
    next(err);
  }
});
